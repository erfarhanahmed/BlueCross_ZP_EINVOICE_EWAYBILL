*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVEINV_API
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVEINV_API          .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVEINV_API'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVEINV_API_EXTRACT
TOTAL = ZVEINV_API_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVEINV_API'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVEINV_API-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVEINV_API_EXTRACT
TOTAL = ZVEINV_API_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVEINV_API-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVEINV_API'
...
TOTAL = ZVEINV_API_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVEINV_API-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVEINV_API-UPD_FLAG NE SPACE.
