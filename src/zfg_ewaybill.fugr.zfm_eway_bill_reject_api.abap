FUNCTION ZFM_EWAY_BILL_REJECT_API.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IM_TOKEN) TYPE  STRING
*"     REFERENCE(IM_API_DATA) TYPE  ZTT_EWAY_BILL_REJECT_STR
*"  EXPORTING
*"     REFERENCE(EX_RETURN) TYPE  STRING
*"     REFERENCE(EX_MESSAGES) TYPE  BAPIRET2_T
*"     REFERENCE(EX_API_DATA) TYPE  ZTT_EWAY_BILL_REJECT_OUT_STR
*"----------------------------------------------------------------------


  DATA:it_api_data TYPE TABLE OF zst_eway_bill_reject_str,
        wa_api_data TYPE zst_eway_bill_reject_str.
  DATA wa_ex_api_data TYPE zst_eway_bill_reject_out_str.
  ...
  IF sy-subrc IS INITIAL.
    ...
    LOOP AT it_api_data INTO wa_api_data.
      ...
      '"data_source":' wa_api_data-data_source "erp"
      '}'
      INTO lv_post SEPARATED BY ' '.
      cl_http_client=>create_by_url(
      EXPORTING
        url = lv_url "'https://client.mastersindia.co/ewayBillReject'
      IMPORTING
...
*           FLAG_NO_LINE_BREAKS       = 'X'
        ...
        ELSEIF lv_ew2 CS 'No content'.
          ...
          lw_message-TYPE = 'S'.
          lw_message-message_v1 = 'E-way Bill rejected successfully'.
