*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVDOC_UOM
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVDOC_UOM           .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVDOC_UOM'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVDOC_UOM_EXTRACT
TOTAL = ZVDOC_UOM_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVDOC_UOM'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVDOC_UOM-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVDOC_UOM_EXTRACT
TOTAL = ZVDOC_UOM_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVDOC_UOM-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVDOC_UOM'
...
TOTAL = ZVDOC_UOM_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVDOC_UOM-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVDOC_UOM-UPD_FLAG NE SPACE.
